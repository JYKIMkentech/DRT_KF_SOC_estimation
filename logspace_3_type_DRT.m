...
% Define θ and τ using logspace
theta_discrete = linspace(log(0.01), log(100), n);  % θ = log(tau)
tau_discrete = exp(theta_discrete);  % tau = exp(theta)
...
% True DRT [Tau,R]
R_discrete_true = normpdf(theta_discrete, log(mu), sigma/mu);  % Adjusted for log-space
...
% Quadprog DRT estimates
R_quad_all = zeros(num_scenarios, n);

% Fmincon DRT estimates
R_fmincon_all = zeros(num_scenarios, n);
...
for s = 1:num_scenarios
    ...
    %% Quadprog Solution
    H_quad = W' * W + lambda * (L' * L);
    f_quad = -W' * V_sd';
    A_quad = -eye(n);
    b_quad = zeros(n, 1);
    options_quad = optimoptions('quadprog', 'Display', 'none');
    R_quad = quadprog(H_quad, f_quad, A_quad, b_quad, [], [], [], [], [], options_quad);
    R_quad(R_quad < 0) = 0;
    R_quad_all(s, :) = R_quad';
    
    %% Fmincon Solution
    objective = @(R) norm(W * R - V_sd', 2)^2 + lambda * norm(L * R, 2)^2;
    lb = zeros(n, 1);
    ub = [];  % No upper bound
    R0_fmincon = zeros(n, 1);
    options_fmincon = optimoptions('fmincon', 'Display', 'none', 'Algorithm', 'interior-point');
    R_fmincon = fmincon(objective, R0_fmincon, [], [], [], [], lb, ub, [], options_fmincon);
    R_fmincon(R_fmincon < 0) = 0;
    R_fmincon_all(s, :) = R_fmincon';
...
for s = 1:num_scenarios
    figure(1 + num_scenarios + s);  % DRT Comparison Figure for each scenario
    hold on;
    ...
    % Plot Quadprog DRT
    plot(tau_discrete, R_quad_all(s, :), '--', 'Color', 'b', 'LineWidth', 1.5, 'DisplayName', 'Quadprog DRT');
    
    % Plot fmincon DRT
    plot(tau_discrete, R_fmincon_all(s, :), '-.', 'Color', 'm', 'LineWidth', 1.5, 'DisplayName', 'fmincon DRT');
    
    hold off;
    set(gca, 'XScale', 'log');  % Use logarithmic scale for tau
    xlabel('\tau (Time Constant, s)');
    ylabel('R (Resistance)');
