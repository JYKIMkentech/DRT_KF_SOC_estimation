...
mu = 10;
sigma = 5;
% 시상수를 linspace 대신 logspace로 변경
tau_discrete = logspace(log10(0.01), log10(20), n); % Discrete tau values on log scale
R_discrete_true = normpdf(log(tau_discrete), log(mu), sigma/mu); % Adjusted for log-space
...
% Aggressive Y-axis zoom: adjust to focus on the lower part of the CVE curve
% 여기서는 자동으로 설정하겠습니다. 필요시 수동으로 조정 가능
% ylim([min(CVE) * 0.9, min(CVE) * 1.1]); 
...
figure;
semilogx(tau_discrete, R_discrete_true, 'k-', 'LineWidth', 2, 'DisplayName', 'True DRT'); hold on;
semilogx(tau_discrete, R_final, 'r--', 'LineWidth', 2, 'DisplayName', 'Optimized DRT');
